...
def test_sdk_minimal_flow(client):
    # Create a library
    lib = client.create_library(LibraryCreate(name="Minimal Test", description="Quick test"))
    assert lib["name"] == "Minimal Test"
    lib_id = UUID(lib["id"])

    # Create a document
    doc = client.create_document(lib_id, DocumentCreate(
        title="Test Doc",
        source="test",
        description="Doc for test"
    ))
    doc_id = UUID(doc["id"])

    # Create a chunk with a fixed embedding (valid 1024D)
    embedding = [0.001 * i for i in range(1024)]
    chunk = client.create_chunk(doc_id, ChunkCreate(
