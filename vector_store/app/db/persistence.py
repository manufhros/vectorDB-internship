...
def save_data(libraries, documents, chunks):
    with open(DATA_DIR / "libraries.json", "w") as f:
        json.dump([lib.model_dump(mode="json") for lib in libraries.values()], f, indent=2)

    with open(DATA_DIR / "documents.json", "w") as f:
        json.dump([doc.model_dump(mode="json") for doc in documents.values()], f, indent=2)


    with open(DATA_DIR / "chunks.json", "w") as f:
        json.dump([chunk.model_dump(mode="json") for chunk in chunks.values()], f, indent=2)
...
def load_data():
    ...
    chunks = {
        UUID(chk["id"]): Chunk(**chk)
        for chk in load_json(DATA_DIR / "chunks.json")
    }
